...
class UNetPrototype(
    ...
    def __init__(
        ...
        self.upfordown3 = torch.nn.ConvTranspose2d(
            num_filters * 8, num_filters * 8, kernel_size=2, stride=2
        )
        self.upfordown2 = torch.nn.ConvTranspose2d(
            num_filters * 4, num_filters * 4, kernel_size=2, stride=2
        )
        self.upfordown1 = torch.nn.ConvTranspose2d(
            num_filters * 2, num_filters * 2, kernel_size=2, stride=2
        )
        self.upforlast = torch.nn.ConvTranspose2d(
    ...
    def forward(self, batch: torch.Tensor) -> torch.Tensor:
        ...
        out = self.upfordown3(out)
        out = torch.cat([out, out_down3], dim=1)

        out = self.up4(out)
        out = self.upfordown2(out)
        out = torch.cat([out, out_down2], dim=1)

        out = self.up3(out)
        out = self.upfordown1(out)
